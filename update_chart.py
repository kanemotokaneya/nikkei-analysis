...
try:
    ...
    close_p, yesterday_p = 38000.0, 38000.0 # 最悪の事態のダミー数値
...
try:
    ...
    # ローソク足がダメなら折れ線グラフで逃げる
    plt.figure(figsize=(12, 8))
    plt.plot(df['Close'].tail(50))
    plt.title("Nikkei 225 Line Chart (Candle Error)")
...
report_content = f"""
    <div class='analysis-box'>
        ...
        <table style='width:100%; border-collapse: collapse; text-align: left;'>
            ...
            <tr><td>日経225(ラージ)</td><td>{oi.get('large_all')}</td><td>{oi.get('large_mar')}</td></tr>
            <tr><td>日経225 mini</td><td>{oi.get('mini_all')}</td><td>{oi.get('mini_mar')}</td></tr>
            <tr><td>TOPIX</td><td>{oi.get('topix_all')}</td><td>{oi.get('topix_mar')}</td></tr>
        </table>
    </div>
"""
